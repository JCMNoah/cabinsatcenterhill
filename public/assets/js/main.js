(function ($) {
  ...
  $(document).ready(function () {
    ...
    function initScrollToTop(retryCount) {
      retryCount = retryCount || 0;
      var progressPath = document.querySelector(".progress-wrap path");
      if (!progressPath) {
        // Retry after a short delay if element doesn't exist yet (for React/Next.js)
        // Maximum 50 retries (5 seconds) to prevent infinite loops
        if (retryCount < 50) {
          setTimeout(function() {
            initScrollToTop(retryCount + 1);
          }, 100);
        }
        return;
      }
    ...
    // Marquee will be initialized by AnimationManager after React hydration
    // This prevents conflicts and ensures elements exist
    function initMarquee() {
      if ($(".marquee").length && $.fn.marquee) {
        $('.marquee').each(function() {
          const $this = $(this);
          // Check if already initialized (marquee plugin adds data attributes)
    ...
    // Marquee initialization is now handled by custom-gsap.js to prevent conflicts
    // setTimeout(initMarquee, 500);
    // Also expose function for AnimationManager to call
    window.initEliteStayMarquee = initMarquee;
  ...
  // Header sticky functionality is now handled by React Header component
  // to avoid conflicts with Next.js DOM manipulation

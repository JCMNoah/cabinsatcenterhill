...
import { User, UserRole } from './src/entities/User.ts'
import { Cabin, CabinStatus } from './src/entities/Cabin.ts'
import { Booking, BookingStatus } from './src/entities/Booking.ts'
import { Review } from './src/entities/Review.ts'
import { Payment, PaymentStatus } from './src/entities/Payment.ts'
...
console.log('🔄 Registering AdminJS TypeORM adapter...')
...
const adminJsConfig = {
  databases: [new Database(AppDataSource)],
  rootPath: '/admin',
  branding: {
        ...
        actions: {
          approveCabin: {
            ...
            handler: async (request, response, context) => {
              ...
              const cabinRepository = AppDataSource.getRepository(Cabin)
              
              await cabinRepository.update(record.params.id, { status: CabinStatus.ACTIVE })
        ...
        actions: {
          markAsPaid: {
            ...
            handler: async (request, response, context) => {
              ...
              await bookingRepository.update(record.params.id, { status: BookingStatus.CONFIRMED })
              ...
              if (existingPayment) {
                await paymentRepository.update(existingPayment.id, { status: PaymentStatus.PAID })
              } else {
